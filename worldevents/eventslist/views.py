...
def home(request,num_events='15'):
   if "event_list" not in request.session: # If there aren't results of a search in session
...
def register(request):
	...
	if form.is_valid():
		...
		messages.success(request, form.cleaned_data['username'] + _(' you have been successfully registered'))
		return HttpResponseRedirect("/")	
    else:
