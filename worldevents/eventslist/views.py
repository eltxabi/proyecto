...
def home(request):
   ...
   if request.method == 'POST':
	    ...
	    nearby_list=Event.objects(Q(title__icontains=title) & Q(category=category) & Q(location__geo_within_sphere=[(float(lat),float(lng)),float(distance)]))
	    form=SearchForm()
	    return render(request, "eventslist/home.html", {'form': form,'nearby_list':nearby_list})  
   else:
        form = SearchForm()
