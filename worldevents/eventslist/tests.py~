...
class HomeTest(LiveServerTestCase):
	...
	response = self.client.get('/')
	num_events = response.content.count('</tr>')
	self.assertEqual(num_events,20)
	expected_input = escape('<td>Concierto59</td>')
	self.assertIn(expected_input,escape(response.content))
	expected_input = escape('<td>Concierto40</td>')
	self.assertIn(expected_input,escape(response.content))
	expected_input = escape('<a href="/eventslist/40">')
	self.assertIn(expected_input,escape(response.content))
	response = self.client.get('/eventslist/40')
	num_events = response.content.count('</tr>')
	self.assertEqual(num_events,20)
	expected_input = escape('<td>Concierto39</td>')
	self.assertIn(expected_input,escape(response.content))
	expected_input = escape('<td>Concierto20</td>')
	self.assertIn(expected_input,escape(response.content))
	expected_input = escape('<a href="/eventslist/60">')
	self.assertIn(expected_input,escape(response.content))
	expected_input = escape('<a href="/eventslist/20">')
	self.assertIn(expected_input,escape(response.content))
	response = self.client.get('/eventslist/20')
	num_events = response.content.count('</tr>')
	self.assertEqual(num_events,20)
	expected_input = escape('<td>Concierto59</td>')
	self.assertIn(expected_input,escape(response.content))
